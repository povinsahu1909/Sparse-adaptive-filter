...
for itr=1:200%run times
    ...
    for j=1:7 %selecting different lms
    ...
    elseif(j==6)
        ...
        for x2=1:length(model)
                ...
                if (model(x2)>(-1/(2*(b-1))) && model(x2)<(1/(2*(b-1))))
                    c(x2)=sign(model(x2)).*(1-(2*b).*abs(model(x2)))./(1+abs(model(x2))).^5;
...
% E4=(err4);
figure;
plot(1:length(x),10*log10(mean(E1)),1:length(x),10*log10(mean(E2)),1:length(x),10*log10(mean(E3)),1:length(x),10*log10(mean(E4)),1:length(x),10*log10(mean(E5)),1:length(x),10*log10(mean(E6)), 1:length(x),10*log10(mean(E7)));
legend('LMS','ZA-LMS','RZA-LMS','BB-ZALMS','BB-RZALMS', 'MBB-ZALMS', 'MBB-RZALMS');
xlabel('Iterations');
ylabel('MSD (dB)');
% figure;
% plot(sys);
